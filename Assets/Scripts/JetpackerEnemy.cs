...
public class JetpackerEnemy : MonoBehaviour {
    ...
    //Health and scoring
    public int health = 5;
    public int enemyScore = 50; // The ammount of score this enemy adds when it dies
    ...
    private void Awake()
    {
        ...
        starting_Point = Random.Range(5, 7); //Setting a random value here
        starting_Points =
    ...
    private void Start()
    {
        time2Move = Random.Range(5.0f, 7.0f); //Giving a random value to time2Move
    ...
    public void MoveToStartingPoint()
    {
        ...
        gameObject.transform.position = starting_Point_Coordinates; //Jetpackers need to start from North and south so no need to move. 
    } //End of MoveToStartingPoint

    public void MoveToFirstRandomPoint()
    {
        ...
        movementDirection =
            Vector2.MoveTowards(thisRigidbody2D.position, random_Point_Coordinates, 2.5f * Time.deltaTime); //Moving towards starting point here
        ...
        if ((thisRigidbody2D.position - random_Point_Coordinates).magnitude < 0.25f)
        {
